...
6. A host API accepting a preset/chunk write is not proof of import. Verify with displayed/formatted values, a native UI check, an exported round-trip, or an audible render gate.
7. Keep exported `.xps` files out of public repos unless they are sanitized and legally safe to publish.
