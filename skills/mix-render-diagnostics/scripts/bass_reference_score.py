#!/usr/bin/env python3
"""Score a bass candidate against a bass reference WAV.

This is a dependency-light bass-specific comparison helper. It deliberately
does not choose verse/chorus/bridge sections automatically; pass already
matched excerpts when section matching matters.
"""
...
try:
    ...
    raise SystemExit("bass_reference_score.py requires numpy") from exc
...
def band_profile(audio: np.ndarray, sample_rate: int) -> dict[str, float]:
...
def analyze(candidate_path: Path, reference_path: Path) -> dict:
    ...
    cand_profile = band_profile(candidate_matched, cand_sr)
    ref_profile = band_profile(reference, ref_sr)
    ...
    candidate_envelope = envelope_metrics(candidate_matched, cand_sr)
    reference_envelope = envelope_metrics(reference, ref_sr)
    ...
    result = {
        ...
        "comparison_duration_seconds": len(candidate) / cand_sr if cand_sr else 0.0,
        "comparison_gates": {
            "duration_mismatch": abs(candidate_original_duration - reference_original_duration) > 0.5,
            "short_comparison": (len(candidate) / cand_sr if cand_sr else 0.0) < 10.0,
        ...
        "attack_body_envelope": {
            "candidate": candidate_envelope,
            "reference": reference_envelope,
        },
        "translation_proxy": translation,
        "mud_penalty": mud_penalty,
        "sub_headroom_risk": sub_headroom,
        "compression_pumping_proxy": compression,
    }
    ...
    result["confidence"] = (
        "low"
        if result["comparison_gates"]["duration_mismatch"]
        or result["comparison_gates"]["short_comparison"]
        or result["comparison_gates"]["low_reference_level"]
        or result["comparison_gates"]["low_candidate_level"]
        else "medium"
    )
    result["notes"] = [
